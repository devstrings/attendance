...
const OvertimeManagement = ({ isManager = false }) => {
  ...
  const fetchCorrectionRequests = async () => {
    try {
      ...
      const res = await fetch(
        `${API}/correction-requests?status=pending&limit=50`,
        {
          headers: { Authorization: `Bearer ${getToken()}` },
        },
      );
  ...
  const handleCorrectionApprove = async () => {
    if (!correctionResolution.trim()) {
      showToast("Resolution note required.", "error");
      return;
    }
    setCorrectionProcessing(correctionModal.req._id);
    try {
      const res = await fetch(
        `${API}/correction-requests/${correctionModal.req._id}/approve`,
        {
          method: "PUT",
          headers: {
  ...
  const handleCorrectionReject = async () => {
    ...
    try {
      const res = await fetch(
        `${API}/correction-requests/${correctionModal.req._id}/reject`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${getToken()}`,
  ...
  const fetchEmployeeAttendance = async (empId, mode) => {
    ...
    try {
      ...
      if (records.length === 0)
        showToast(
          `No records found for ${mode === "today" ? "today" : "last 7 days"}.`,
          "error",
        );
    } catch (e) {
  ...
  const handleDirectSet = async () => {
    if (!selectedRecord || !directMinutes || parseInt(directMinutes) <= 0) {
      showToast(
        "Please select an attendance record and enter overtime minutes.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        showToast(
          `✅ Overtime set: ${minsToHrs(parseInt(directMinutes))} for ${selectedEmployee?.firstName}`,
        );
        setSelectedRecord(null);
        setDirectMinutes("");
        setDirectNote("");
    ...
    try {
      const body =
        action === "approve"
          ? {
              approved: true,
              overtimeMinutes: approveMinutes
                ? parseInt(approveMinutes)
                : undefined,
            }
  ...
  const handleFixSubmit = async () => {
    ...
    try {
      const dateStr = new Date(fixSelectedRecord.date)
        .toISOString()
        .split("T")[0];
      const body = {
        status: fixForm.status,
        clockIn: `${dateStr}T${fixForm.clockIn}:00+05:00`,
        clockOut: fixForm.clockOut
          ? `${dateStr}T${fixForm.clockOut}:00+05:00`
          : null,
        correctionReason: fixForm.reason,
      };
      const res = await fetch(`${API}/attendance/${fixSelectedRecord._id}/correct`, {
  ...
  return (
    <div className={isManager ? "manager-container" : "admin-container"}>
      ...
      <div className={isManager ? "manager-layout" : "admin-layout"}>
        <AdminSidebar />
        <div
          className={isManager ? "manager-content" : "admin-content"}
        ...
        >
          ...
          <div style={S.pageHeader}>
            <div>
              ...
              <p style={S.pageSub}>
                Review overtime requests, correction requests, and fix
                attendance directly
              </p>
            </div>
          </div>
          ...
          <div style={S.tabs}>
            ...
            <button
              style={{
                ...S.tab,
                ...(activeTab === "corrections" ? S.tabActive : {}),
              }}
            ...
            <button
              style={{ ...S.tab, ...(activeTab === "set" ? S.tabActive : {}) }}
              onClick={() => setActiveTab("set")}
            >
              ➕ Set Overtime Directly
            </button>
            <button
              style={{ ...S.tab, ...(activeTab === "fix" ? S.tabActive : {}) }}
            ...
            <div style={S.card}>
              ...
              >
                <div style={S.cardTitle}>
                  Pending Employee Overtime Requests
                </div>
                <button style={S.refreshBtn} onClick={fetchPending}>
                  🔄 Refresh
                </button>
              </div>
              {loading ? (
                ...
                <div style={S.emptyBox}>
                  <div style={{ fontSize: 52, opacity: 0.3, marginBottom: 10 }}>
                    ✅
                  </div>
                  <div
                    style={{
                        ...
                        >
                          <div style={{ flex: 1 }}>
                            ...
                            >
                              ...
                              &nbsp;·&nbsp; Regular:{" "}
                              <strong>{req.workHours || 0} hrs</strong>
                            </div>
                            <div
                              style={{
                                display: "flex",
                                gap: 12,
                                flexWrap: "wrap",
                                marginBottom: req.overtimeNote ? 10 : 0,
                              }}
                            >
                              <div
                                style={{
                              ...
                              <div style={S.infoBox}>
                                ...
                                <div style={S.infoVal}>
                                  {req.overtimeRequestedAt
                                    ? fmtDate(req.overtimeRequestedAt)
                                    : "—"}
                                </div>
                              </div>
                            </div>
                          ...
                          >
                            <button
                              ...
                              onClick={() => {
                                setApproveModal({ req, action: "approve" });
                                setApproveMinutes(String(req.overtimeMinutes));
                              }}
                            >
                              ✅ Approve
                            </button>
                            <button
                              ...
                              disabled={processing === req._id}
                              onClick={() =>
                                setApproveModal({ req, action: "reject" })
                              }
                            >
                              ❌ Reject
                            </button>
          ...
          {activeTab === "corrections" && (
            <div style={S.card}>
              ...
              >
                ...
                <button style={S.refreshBtn} onClick={fetchCorrectionRequests}>
                  🔄 Refresh
                </button>
              </div>
              {correctionLoading ? (
                      ...
                      >
                        <div style={{ flex: 1 }}>
                          ...
                          >
                            ...
                            Issue:{" "}
                            <strong>{req.issueType?.replace(/_/g, " ")}</strong>
                          </div>
                        ...
                        >
                          <button
                            style={S.approveBtn}
                            disabled={correctionProcessing === req._id}
                            onClick={() => {
                              setCorrectionModal({ req, action: "approve" });
                              setCorrectionResolution("");
                            }}
                          >
                            ✅ Approve
                          ...
                          <button
                            ...
                            disabled={correctionProcessing === req._id}
                            onClick={() => {
                              setCorrectionModal({ req, action: "reject" });
                              setCorrectionResolution("");
                            }}
                          >
                            ❌ Reject
          ...
          {activeTab === "set" && (
            <div style={S.card}>
              ...
              <p
                style={{ fontSize: 13, color: "#6b7280", margin: "8px 0 20px" }}
              >
                Set overtime for an employee directly. They will be notified
                automatically.
              </p>
              ...
              {selectedEmployee && (
                <>
                  <div style={{ marginBottom: 16 }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      <button
                        style={{
                          ...S.modeBtn,
                          ...(dateMode === "today" ? S.modeBtnActive : {}),
                        }}
                  ...
                  {loadingRecords ? (
                    ...
                    <div style={S.formGroup}>
                      <label style={S.label}>
                        ...
                        <span style={{ color: "#9ca3af", fontWeight: 400 }}>
                          ({attendanceRecords.length} found)
                        </span>
                      </label>
                            ...
                            >
                              <div style={{ flex: 1 }}>
                                ...
                                <div style={{ fontSize: 12, color: "#6b7280" }}>
                                  In: {fmtTime(r.clockIn)} · Out:{" "}
                                  {fmtTime(r.clockOut)} · {r.workHours || 0} hrs
                                </div>
                                {hasOT && (
                                  <div
                                    style={{
                                  ...
                                  >
                                    ⏱️ OT already: {r.overtimeStatus} (
                                    {minsToHrs(r.overtimeMinutes)})
                                  </div>
                                )}
                              </div>
                              {sel && (
                    ...
                    >
                      ❌ No records found for {selectedEmployee.firstName} in{" "}
                      {dateMode === "today" ? "today" : "last 7 days"}.
                    </div>
                  )}
                </>
              )}
              ...
              {selectedRecord && (
                <>
                  ...
                  >
                    ...
                    >
                      In: {fmtTime(selectedRecord.clockIn)} · Out:{" "}
                      {fmtTime(selectedRecord.clockOut)} ·{" "}
                      {selectedRecord.workHours || 0} hrs
                    </div>
                  </div>
                  <div style={S.formGroup}>
                      ...
                      {[30, 60, 90, 120].map((m) => (
                        <button
                          ...
                          style={{
                            ...S.quickBtn,
                            background:
                              directMinutes === String(m)
                                ? "#667eea"
                                : "#f3f4f6",
                            color:
                              directMinutes === String(m) ? "white" : "#374151",
                          }}
                        >
                          {minsToHrs(m)}
                        </button>
                    ...
                    {directMinutes > 0 && (
                      <div
                        style={{ fontSize: 12, color: "#667eea", marginTop: 6 }}
                      >
                  ...
                  <div style={S.formGroup}>
                    ...
                    <input
                      placeholder="e.g. Project deadline overtime"
                      value={directNote}
                  ...
                  >
                    ...
                    <button
                      style={{
                        ...S.setBtn,
                        opacity: !directMinutes || directSubmitting ? 0.5 : 1,
                      }}
          ...
          {activeTab === "fix" && (
            <div style={S.card}>
              ...
              <p
                style={{ fontSize: 13, color: "#6b7280", margin: "8px 0 20px" }}
              >
                Directly correct any employee's attendance record for the last 7
                days.
              </p>
              ...
              <div style={S.formGroup}>
                ...
                <div style={{ position: "relative" }} ref={fixSearchRef}>
                  <input
                    ...
                    onChange={(e) => handleFixSearchInput(e.target.value)}
                    onFocus={() =>
                      fixSuggestions.length > 0 && setShowFixSuggestions(true)
                    }
                    autoComplete="off"
                  />
                    ...
                    <div style={S.dropdown}>
                      {fixSuggestions.map((emp) => (
                        <div
                          key={emp._id}
                          style={S.dropItem}
                          onMouseDown={() => handleSelectFixEmployee(emp)}
                        >
                          <div style={S.dropAvatar}>
                            {emp.firstName?.[0]}
                            {emp.lastName?.[0]}
              ...
              {fixEmployee && (
                <>
                  <div style={{ marginBottom: 16 }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      <button
                        style={{
                          ...S.modeBtn,
                          ...(fixDateMode === "today" ? S.modeBtnActive : {}),
                        }}
                        onClick={() => handleFixDateModeChange("today")}
                      >
                        📅 Today
                      </button>
                      <button
                        style={{
                          ...S.modeBtn,
                          ...(fixDateMode === "week" ? S.modeBtnActive : {}),
                        }}
                        onClick={() => handleFixDateModeChange("week")}
                      >
                        📆 Last 7 Days
                      </button>
                    </div>
                  ...
                  {fixLoadingRecords ? (
                    ...
                    <div style={S.formGroup}>
                      <label style={S.label}>
                        Select Record to Fix{" "}
                        <span style={{ color: "#9ca3af", fontWeight: 400 }}>
                          ({fixRecords.length} found)
                        </span>
                      </label>
                      <div style={S.resultList}>
                        {fixRecords.map((r) => {
              ...
              {fixSelectedRecord && (
                <>
                  ...
                  >
                    <div style={{ fontWeight: 600, color: "#374151" }}>
                      Editing: {fixEmployee?.firstName} {fixEmployee?.lastName}{" "}
                      — {fmtDate(fixSelectedRecord.date)}
                    </div>
                  </div>
                  ...
                  >
                    <div style={S.formGroup}>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setFixForm((f) => ({ ...f, status: e.target.value }))
                        }
                        style={S.input}
                      >
                    ...
                    <div style={S.formGroup}>
                      ...
                      <input
                        type="time"
                        value={fixForm.clockIn}
                        onChange={(e) =>
                          setFixForm((f) => ({ ...f, clockIn: e.target.value }))
                        }
                        style={S.input}
                      />
                    </div>
                    ...
                    <div style={S.formGroup}>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFixForm((f) => ({ ...f, reason: e.target.value }))
                        }
                        style={S.input}
                      />
                    </div>
                  ...
                  >
                    <button
                      ...
                      onClick={() => setFixSelectedRecord(null)}
                    >
                      Cancel
                    </button>
                    <button
                      style={{ ...S.setBtn, opacity: fixSubmitting ? 0.5 : 1 }}
          ...
          {approveModal && (
            <div style={S.overlay} onClick={() => setApproveModal(null)}>
              <div style={S.modal} onClick={(e) => e.stopPropagation()}>
                ...
                >
                  <strong>
                    {approveModal.req.employeeId?.firstName}{" "}
                    {approveModal.req.employeeId?.lastName}
                  </strong>{" "}
                  — {fmtDate(approveModal.req.date)}
                  <br />
                  Requested:{" "}
                  <strong>{minsToHrs(approveModal.req.overtimeMinutes)}</strong>
                  {approveModal.req.overtimeNote && (
                    <>
                      <br />
                      Note: {approveModal.req.overtimeNote}
                    </>
                  )}
                </div>
                {approveModal.action === "approve" ? (
                  <>
                    <label style={S.label}>
                      Approved Minutes (adjust if needed)
                    </label>
                    <input
                      type="number"
          ...
          {correctionModal && (
            <div style={S.overlay} onClick={() => setCorrectionModal(null)}>
              <div style={S.modal} onClick={(e) => e.stopPropagation()}>
                ...
                >
                  <strong>{correctionModal.req.employeeName}</strong> —{" "}
                  {fmtDate(correctionModal.req.attendanceDate)}
                  <br />
                  Change: <strong>
                    {correctionModal.req.currentStatus}
                  </strong> →{" "}
                  <strong>{correctionModal.req.requestedStatus}</strong>
                  <br />
                ...
                >
                  ...
                  {correctionModal.action === "approve" ? (
                    <button
                      style={S.approveBtn}
                      onClick={handleCorrectionApprove}
                    ...
                    <button
                      style={S.rejectBtn}
                      onClick={handleCorrectionReject}
                      disabled={!!correctionProcessing}
                    >
