...
const AttendanceDetails = () => {
    ...
    try {
      ...
      if (response.data.success && response.data.data.attendance) {
        ...
        setRecord({
          ...
          date:         att.date,
          status:       att.status,
          clockIn:      att.clockIn  ? new Date(att.clockIn).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }) : null,
          clockOut:     att.clockOut ? new Date(att.clockOut).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }) : null,
    ...
    try {
      ...
      try {
        ...
        if (listRes.data.success) {
          employee = listRes.data.data.employees.find(
            emp => emp._id === employeeId || emp._id?.toString() === employeeId
          );
        }
      }
      ...
      setRecord({
        ...
        managedBy:    todayRecord?.managerId
          ...
          : (employee.managerId ? 'Assigned' : 'N/A')
      });
  ...
  const getStatusBadgeStyle = (status) => {
    const map = {
      ...
      not_marked: { background: '#f9fafb', color: '#6b7280', border: '1px solid #e5e7eb' },
    };
    return { ...badgeBase, ...(map[status] || { background: '#f3f4f6', color: '#6b7280' }) };
  };
  ...
  while (cursor <= end) {
    ...
    if (day !== 0 && day !== 6) workingDaysCount++; // Mon-Fri
    cursor.setDate(cursor.getDate() + 1);
  }

  const presentDays = employeeHistory.filter(h =>
    ['present', 'late', 'half-day'].includes(h.status)).length;
  const leaveDays   = employeeHistory.filter(h =>
    ['on-leave', 'leave'].includes(h.status)).length;
  ...
  return { totalDays: workingDaysCount, presentDays, absentDays, leaveDays, lateDays };
};
      ...
      <div className="admin-container">
        ...
        <div className="admin-layout">
          ...
          <div className="admin-content" style={S.center}>
            <div style={S.spinner} /><style>{spinCSS}</style>
            <p style={{ color: '#6b7280', marginTop: 12 }}>Loading attendance details...</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="admin-layout">
          ...
          <div className="admin-content" style={S.center}>
            <div style={{ textAlign: 'center' }}>
              ...
              <h2 style={{ color: '#111827', marginBottom: 8 }}>{error || 'Record Not Found'}</h2>
              <p style={{ color: '#6b7280', marginBottom: 24 }}>The attendance record could not be found.</p>
        ...
        <div className="admin-content" style={{ padding: 24, background: '#f9fafb', minHeight: '100vh' }}>
          ...
          <div style={S.pageHeader}>
            <div>
              ...
              <p style={S.pageSub}>{record.employeeName} · {record.employeeCode}</p>
            </div>
          ...
          <div style={S.card}>
            ...
            <div style={S.infoGrid}>
              {[
                ...
                { label: 'Joining Date', value: record.joiningDate ? new Date(record.joiningDate).toLocaleDateString('en-GB') : 'N/A' },
              ].map(item => (
          ...
          <div style={S.card}>
            ...
            <div style={S.todayGrid}>
              <div style={S.todayItem}>
                ...
                <div style={S.infoValue}>{new Date(record.date).toLocaleDateString('en-GB', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' })}</div>
              </div>
              <div style={S.todayItem}>
                <div style={S.infoLabel}>Status</div>
                <span style={getStatusBadgeStyle(record.status)}>
                  {getStatusIcon(record.status)} {record.status === 'not_marked' ? 'Not Marked' : record.status}
                </span>
              </div>
              <div style={S.todayItem}>
                <div style={S.infoLabel}>Clock In</div>
                <div style={{ ...S.infoValue, color: record.clockIn ? '#059669' : '#9ca3af' }}>
                  {record.clockIn || '—'}
                </div>
              </div>
              <div style={S.todayItem}>
                <div style={S.infoLabel}>Clock Out</div>
                <div style={{ ...S.infoValue, color: record.clockOut ? '#3b82f6' : '#9ca3af' }}>
                  {record.clockOut || '—'}
                </div>
              </div>
              <div style={S.todayItem}>
              ...
              <div style={S.todayItem}>
                ...
                <div style={{ ...S.infoValue, color: record.notes ? '#374151' : '#9ca3af', fontSize: 13 }}>
          ...
          <div style={S.card}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 14, fontWeight: 600, color: '#374151' }}>Report Period:</span>
              {[
                ...
                <button key={t.key}
                  ...
                  onClick={() => { setReportType(t.key); if (t.key !== 'custom') setCustomDates({ startDate: '', endDate: '' }); }}>
                  {t.label}
                </button>
              ))}
              {reportType === 'custom' && (
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                  <input type="date" value={customDates.startDate}
                    onChange={e => setCustomDates(p => ({ ...p, startDate: e.target.value }))}
                    style={S.dateInput} />
                  <span style={{ color: '#6b7280' }}>to</span>
                  <input type="date" value={customDates.endDate}
                    onChange={e => setCustomDates(p => ({ ...p, endDate: e.target.value }))}
                    style={S.dateInput} />
                  <button style={S.typeBtnActive}
          ...
          <div style={{ ...S.card, padding: 0, overflow: 'hidden' }}>
            <div style={{ padding: '18px 24px', borderBottom: '1px solid #f3f4f6' }}>
              <div style={S.cardTitle}>
                📅 Attendance History ({reportType === 'weekly' ? 'Last 7 Days' : reportType === 'monthly' ? 'Last 30 Days' : 'Custom Range'})
              </div>
            </div>
            ...
            {historyLoading ? (
              ...
              <div style={{ overflowX: 'auto' }}>
                <table style={S.table}>
                  <thead>
                    <tr style={S.theadRow}>
                      {['Date', 'Status', 'Clock In', 'Clock Out', 'Hours', 'Late', 'Remarks'].map(h => (
                  ...
                  <tbody>
                    {employeeHistory.map((h, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid #f3f4f6' }}
                        ...
                        <td style={S.td}>{new Date(h.date).toLocaleDateString('en-GB')}</td>
                        <td style={S.td}>
                          <span style={getStatusBadgeStyle(h.status)}>
                            {getStatusIcon(h.status)} {h.status}
                          </span>
                        </td>
                        <td style={{ ...S.td, color: h.clockIn !== '-' ? '#059669' : '#9ca3af' }}>{h.clockIn}</td>
                        <td style={{ ...S.td, color: h.clockOut !== '-' ? '#3b82f6' : '#9ca3af' }}>{h.clockOut}</td>
                        <td style={S.td}><strong>{h.hoursWorked} hrs</strong></td>
                        <td style={S.td}>
                          {h.isLate
                            ? <span style={{ background: '#fef3c7', color: '#92400e', padding: '2px 8px', borderRadius: 12, fontSize: 12, fontWeight: 600 }}>
                                ⏰ {h.lateMinutes} min
                              </span>
                            : <span style={{ color: '#10b981', fontSize: 12, fontWeight: 600 }}>✅ On Time</span>
                          }
                        </td>
                        <td style={{ ...S.td, color: '#6b7280', fontSize: 12, maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          {h.remarks}
                        </td>
                      </tr>
                    ))}
              ...
              <div style={{ textAlign: 'center', padding: '48px', color: '#9ca3af' }}>
                ...
                <div style={{ fontSize: 13, marginTop: 4 }}>No attendance records for the selected period.</div>
              </div>
            )}
          </div>
...
const S = {
  ...
  backBtn:    { padding: '10px 20px', background: 'white', border: '2px solid #e5e7eb', borderRadius: 10, cursor: 'pointer', fontSize: 14, fontWeight: 600, color: '#374151' },
  ...
  infoItem:   { padding: '12px 16px', background: '#f9fafb', borderRadius: 10, border: '1px solid #f3f4f6' },
  infoLabel:  { fontSize: 11, color: '#9ca3af', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 4 },
  infoValue:  { fontSize: 14, fontWeight: 600, color: '#111827' },

  // Today Grid
  todayGrid:  { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16 },
  todayItem:  { padding: '12px 16px', background: '#f9fafb', borderRadius: 10, border: '1px solid #f3f4f6' },

  // Report type buttons
  typeBtn:      { padding: '8px 16px', border: '2px solid #e5e7eb', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', background: 'white', color: '#6b7280' },
  typeBtnActive:{ padding: '8px 16px', border: '2px solid #667eea', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', background: '#667eea', color: 'white' },
  dateInput:    { padding: '8px 12px', border: '2px solid #e5e7eb', borderRadius: 8, fontSize: 13, outline: 'none' },
  ...
  statCard:   { background: 'white', borderRadius: 12, padding: '16px 20px', boxShadow: '0 1px 3px rgba(0,0,0,0.07)', borderTop: '3px solid', textAlign: 'center' },
  statValue:  { fontSize: 32, fontWeight: 800, lineHeight: 1, marginBottom: 6 },
  statLabel:  { fontSize: 12, color: '#6b7280', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.4px' },

  // Table
  table:      { width: '100%', borderCollapse: 'collapse', fontSize: 13 },
  theadRow:   { background: 'linear-gradient(135deg, #667eea, #764ba2)' },
  th:         { padding: '12px 16px', textAlign: 'left', color: 'white', fontWeight: 700, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.5px', whiteSpace: 'nowrap' },
  td:         { padding: '12px 16px', color: '#374151', verticalAlign: 'middle' },

  spinner:    { width: 40, height: 40, border: '4px solid #e5e7eb', borderTop: '4px solid #667eea', borderRadius: '50%', animation: 'spin 0.8s linear infinite' },
};
