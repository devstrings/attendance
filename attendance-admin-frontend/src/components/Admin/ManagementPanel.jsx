...
const ManagementPanel = () => {
  ...
  const validateHolidayForm = () => {
    const errors = {};
    if (!holidayForm.holidayDate) errors.holidayDate = 'Holiday date is required';
    if (!holidayForm.holidayName.trim()) errors.holidayName = 'Holiday name is required';
  ...
  const handleConfigSubmit = async (e) => {
    ...
    try {
      ...
      if (systemConfig && systemConfig._id) {
        response = await adminService.updateSystemConfig(systemConfig._id, configData);
      } else {
                    ...
                    <div className="form-row">
                      <div className="form-group">
                        ...
                        {holidayErrors.holidayDate && (
                          <span className="error-text">{holidayErrors.holidayDate}</span>
                        )}
                      </div>
                      ...
                      <div className="form-group">
                        ...
                        {holidayErrors.holidayName && (
                          <span className="error-text">{holidayErrors.holidayName}</span>
                        )}
                      </div>
                ...
                <div className="config-card">
                  ...
                  {holidays.length > 0 ? (
                      ...
                      {holidays.map((holiday) => (
                        <div key={holiday._id} className="holiday-card">
                          <div className="holiday-date">
                            <div className="date-day">{new Date(holiday.date).getDate()}</div>
                            <div className="date-month">{getMonthName(holiday.date)}</div>
                          </div>
                          <div className="holiday-info">
                            <h3>{holiday.name}</h3>
                            <p>{formatDate(holiday.date)} • {holiday.year}</p>
                          </div>
                          <button
                    ...
                    <div className="empty-state">
                      <p>📅 No holidays configured. Add your first holiday above!</p>
                    </div>
                  )}
                </div>
              </div>
                ...
                <form onSubmit={handleConfigSubmit}>
                  <div className="config-card">
                    ...
                    <p className="card-description">
                      Select which days are working days. Changes apply to all employees.
                    </p>
                        ...
                        <div className="days-grid">
                          {allDays.map((day) => (
                            <label key={day} className="day-checkbox">
                              ...
                              />
                              <span className={configForm.workingDays.includes(day) ? 'selected' : ''}>
                                {day}
                              </span>
                            </label>
                          ))}
                    ...
                    <div className="info-box">
                      <strong>Selected Working Days:</strong> {configForm.workingDays.join(', ')}
                    </div>

                    <button type="submit" className="btn-submit-full" disabled={submitting}>
              ...
              <div className="tab-panel">
                <form onSubmit={handleConfigSubmit}>
                  <div className="config-card">
                    ...
                    <p className="card-description">
                      Set office timing, late entry threshold, and break duration.
                    </p>
                    ...
                    <div className="info-box">
                      <strong>Summary:</strong> Office hours {configForm.startTime} - {configForm.endTime} 
                      ({((new Date(`1970-01-01T${configForm.endTime}`) - new Date(`1970-01-01T${configForm.startTime}`)) / (1000 * 60 * 60)).toFixed(1)} hours)
                      with {configForm.breakTime} min break
                    </div>
                  ...
                  <div className="config-card">
                    ...
                    <div className="form-grid">
                      ...
                      <div className="form-group">
                        ...
                        <small>Automatically mark as absent if leaves exceed the limit</small>
                      </div>
                    </div>

                    <div className="info-box warning">
                      <strong>⚠️ Policy:</strong> Employees can take {configForm.allowedLeaves} leave(s) per month.
                      {configForm.autoAbsentOnExceed && ` Leave #${configForm.allowedLeaves + 1} onwards will be marked as Absent.`}
                    </div>
