...
const Settings = () => {
  ...
  const loadAdminProfile = async () => {
    ...
    try {
      // 1. Try API first — most up-to-date data
      const response = await api.get('/admin/profile');
      if (response.data?.success) {
        const u = response.data.data?.admin || response.data.data || {};
        setProfileForm({
      ...
      console.warn('API profile fetch failed, trying localStorage', e);
    }

    // 2. Fallback — admin_user from localStorage (set at login)
    try {
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        if (stored) {
          const u = JSON.parse(stored);
          localStorage.setItem('admin_user', JSON.stringify({ ...u, ...profileForm }));
        }
  ...
  const handleSystemSubmit = async (e) => {
    ...
    try {
      ...
      showError(error.response?.data?.message || 'Failed to save system settings');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="admin-container">
      ...
      <div className="admin-layout">
        ...
        <div className="admin-content">
          ...
          {successMsg && (
            <div style={banner('success')}>{successMsg}</div>
          )}
          {errorMsg && (
            <div style={banner('error')}>{errorMsg}</div>
          )}
                  ...
                  <form onSubmit={handleProfileSubmit} className="settings-form">
                    <div className="form-grid">
                      ...
                      <div className="form-group full-width">
                        <label>Address</label>
                        <textarea name="address" value={profileForm.address}
                  ...
                  <div className="form-grid">
                    <div className="form-group">
                      ...
                      <input type="text" name="companyName"
                        value={systemSettings.companyName} onChange={handleSystemChange} required />
                    </div>
                    <div className="form-group">
                      <label>Working Hours Start *</label>
                      <input type="time" name="workingHoursStart"
                        value={systemSettings.workingHoursStart} onChange={handleSystemChange} required />
                    </div>
                    <div className="form-group">
                      <label>Working Hours End *</label>
                      <input type="time" name="workingHoursEnd"
                        value={systemSettings.workingHoursEnd} onChange={handleSystemChange} required />
                    </div>
                    <div className="form-group">
                      <label>Overtime Rate (multiplier)</label>
                      <input type="number" name="overtimeRate"
                        value={systemSettings.overtimeRate} onChange={handleSystemChange}
                        step="0.1" min="1" />
                    </div>
                    <div className="form-group">
                      <label>Late Marking Grace (minutes)</label>
                      <input type="number" name="lateMarkingMinutes"
                        value={systemSettings.lateMarkingMinutes} onChange={handleSystemChange} min="0" />
                    </div>
                    <div className="form-group checkbox-group">
                      <label>
                        <input type="checkbox" name="emailNotifications"
                          checked={systemSettings.emailNotifications} onChange={handleSystemChange} />
                    ...
                    <div className="form-group checkbox-group">
                      <label>
                        <input type="checkbox" name="smsNotifications"
                          checked={systemSettings.smsNotifications} onChange={handleSystemChange} />
