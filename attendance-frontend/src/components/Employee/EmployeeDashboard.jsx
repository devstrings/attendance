  ...
  if (loading) {
    return (
      <div className="employee-container">
        <EmployeeNavbar />
        <div className="employee-content" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '80vh' }}>
          <div style={s.spinner} />
        </div>
  ...
  return (
    <div className="employee-container">
      ...
      <div className="employee-content" style={s.content}>
        ...
        <div style={s.header}>
          <div>
            <h1 style={s.title}>Welcome back, {employee.firstName} {employee.lastName}! 👋</h1>
            <p style={s.subtitle}>Here's your attendance overview</p>
            <span style={s.refreshBadge}>🔄 Auto-refreshing every 10 seconds</span>
          </div>
          <div style={s.headerRight}>
            <div style={s.workingDaysBig}>
              ...
              {joinDate && (
                <span style={s.wdSince}>since {joinDate.toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
              )}
            </div>
          </div>
        </div>
          ...
          {todayCards.map(card => (
            <div
              ...
              style={{
                ...
                boxShadow: card.active ? `0 4px 20px ${card.color}25` : '0 1px 3px rgba(0,0,0,0.05)',
                opacity: (!card.active && todayStatus !== 'not_marked') ? 0.55 : 1,
              }}
            >
              <div style={{ fontSize: card.active ? '34px' : '26px', transition: 'font-size 0.3s', filter: card.active ? 'none' : 'grayscale(0.5)' }}>
                {card.icon}
              </div>
              <div style={s.todayInfo}>
                <div style={{ ...s.todayCardLabel, color: '#9ca3af', fontSize: '11px' }}>{card.label}</div>
                <div style={{ ...s.todayStatus, color: card.active ? card.color : '#c4c9d4', fontSize: card.active ? '15px' : '13px', fontWeight: card.active ? '700' : '500' }}>
        ...
        <div style={s.middleGrid}>
            ...
            <div style={s.chartBody}>
              ...
              <div style={s.ringsWrap}>
                ...
                <div style={{ position: 'absolute', top: 0, left: 0 }}>
                  <CircleRing percentage={presentPct} color="#10b981" size={160} stroke={14} />
                </div>
                {/* Middle: Absent */}
                <div style={{ position: 'absolute', top: 16, left: 16 }}>
                  <CircleRing percentage={absentPct} color="#ef4444" size={128} stroke={12} />
                </div>
                {/* Inner: Leave */}
                <div style={{ position: 'absolute', top: 30, left: 30 }}>
                  <CircleRing percentage={leavePct} color="#f59e0b" size={100} stroke={10} />
                </div>
                {/* Center text */}
          ...
          <div style={s.actionsCard}>
            ...
            <div style={s.actionsList}>
              {[
                { icon: '📝', label: 'View My Attendance', desc: 'Check this month', color: '#10b981', path: '/employee/my-attendance' },
                { icon: '📅', label: 'Attendance History',  desc: 'Past records',     color: '#3b82f6', path: '/employee/attendance-history' },
                { icon: '🏖️', label: 'Request Leave',       desc: 'Apply for time off', color: '#f59e0b', path: '/employee/request-leave' },
                { icon: '⚠️', label: 'Report Issue',        desc: 'Flag a problem',   color: '#ef4444', path: '/employee/report-issue' },
                { icon: '👤', label: 'My Profile',          desc: 'View & edit info', color: '#8b5cf6', path: '/employee/profile' },
              ].map(a => (
                <div key={a.label} style={s.actionRow} onClick={() => navigate(a.path)}>
                  <div style={{ ...s.actionIconBox, background: `${a.color}15`, color: a.color }}>{a.icon}</div>
                  <div style={{ flex: 1 }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={s.table}>
              ...
              <tbody>
                ...
                {todayRecord && (
                  <tr style={{ ...s.tr, background: '#f0f9ff', borderLeft: '3px solid #3b82f6' }}>
                    <td style={s.td}>
                      <span style={{ fontWeight: '700', color: '#2563eb' }}>Today</span>
                    ...
                    <td style={s.td}>
                      <span style={{ ...s.badge, ...badgeStyle(todayRecord.status) }}>
                        ...
                        {isLateToday && <span style={{ marginLeft: '4px', fontSize: '10px' }}>⏰</span>}
                      </span>
                    </td>
                    <td style={s.td}>
                      {todayRecord.clockIn
                  ...
                  ? recentAttendance
                      ...
                      .map(r => (
                        <tr key={r._id} style={s.tr}>
                          ...
                          <td style={s.td}>
                            <span style={{ ...s.badge, ...badgeStyle(r.status) }}>
                              {statusIcon(r.status)} {r.status}
                            </span>
                          </td>
                          <td style={s.td}>{r.clockIn  ? new Date(r.clockIn ).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '—'}</td>
                          <td style={s.td}>{r.clockOut ? new Date(r.clockOut).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '—'}</td>
                          <td style={s.td}>{r.workHours || 0} hrs</td>
                        </tr>
                      ))
                  : !todayRecord && (
                    <tr><td colSpan="5" style={s.noData}>No attendance records yet</td></tr>
                  )}
              </tbody>
            </table>
          </div>
        ...
        {systemConfig && (
          <div style={{ ...s.tableCard, marginTop: '20px' }}>
            ...
            <div style={s.policyGrid}>
              {[
                { icon: '🕙', label: 'Office Hours', val: `${formatTime(systemConfig.workingHours?.startTime)} – ${formatTime(systemConfig.workingHours?.endTime)}` },
                { icon: '⏰', label: 'Late After',   val: formatTime(systemConfig.workingHours?.lateEntryTime) },
                { icon: '📅', label: 'Working Days', val: systemConfig.workingDays?.join(', ') || 'Mon–Fri' },
                { icon: '🏖️', label: 'Weekends',    val: systemConfig.weekendDays?.join(', ') || 'Sat & Sun' },
                { icon: '☕', label: 'Break Time',   val: `${systemConfig.breakTime || 60} min` },
                { icon: '📝', label: 'Leaves/Month', val: `${systemConfig.leavePolicy?.allowedLeaves || 2} days` },
              ].map(p => (
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
const statusIcon = (s) => ({ present: '✅', absent: '❌', late: '⏰', leave: '🏖️' }[s] || '');
...
const s = {
  ...
  title:   { fontSize: '24px', fontWeight: '700', color: '#fff', margin: '0 0 6px 0' },
  ...
  wdNumber: { fontSize: '40px', fontWeight: '800', color: '#fff', lineHeight: 1 },
  wdLabel:  { fontSize: '12px', color: '#c7d2fe', fontWeight: '600', marginTop: '4px' },
  ...
  ringPct: { display: 'block', fontSize: '26px', fontWeight: '800', color: '#111827' },
  ringLbl: { display: 'block', fontSize: '11px', color: '#6b7280', fontWeight: '600' },

  // Legend
  legend: { flex: 1, minWidth: '160px', display: 'flex', flexDirection: 'column', gap: '10px' },
  legendRow: { display: 'flex', alignItems: 'center', gap: '8px' },
  legendDot: { width: '10px', height: '10px', borderRadius: '50%', flexShrink: 0 },
  legendLabel: { flex: 1, fontSize: '13px', color: '#374151', fontWeight: '500' },
  legendVal: { fontSize: '13px', color: '#111827', fontWeight: '600' },
  legendPct: { fontSize: '12px', fontWeight: '700', minWidth: '34px', textAlign: 'right' },
  ...
  actionsList: { padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: '4px' },
  actionRow: {
  ...
  policyLabel: { fontSize: '11px', color: '#6b7280', fontWeight: '600', marginBottom: '3px' },
