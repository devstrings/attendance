...
const NewCorrectionModal = ({ onClose, onSubmitted }) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (record) {
          ...
          if (record.clockOut) {
            clockOutTime = new Date(record.clockOut).toLocaleTimeString(
              "en-GB",
  ...
  const showClockFields =
    form.requestedStatus === "present" ||
    form.requestedStatus === "half-day" ||
    form.requestedStatus === "late";
          ...
          <div style={ms.row}>
            <div style={ms.field}>
              ...
              <div style={{
                ...
                {form.currentStatus === 'absent' ? '❌' : form.currentStatus === 'present' ? '✅' : '⏰'} {form.currentStatus?.toUpperCase()}
                {fetchingAttendance && <span style={{fontSize:11, color:'#94a3b8'}}>⏳ loading...</span>}
              </div>
            </div>
            <div
              style={{
...
const MyRequests = () => {
  ...
  const [activeTab, setActiveTab] = useState(
    location.state?.activeTab || "leave",
  );
  ...
  const [showCorrectionModal, setShowCorrectionModal] = useState(
    location.state?.openModal || false,
  );
  const [successMsg, setSuccessMsg] = useState("");
  const [dismissedIds, setDismissedIds] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("dismissedLeaveIds") || "[]");
    } catch {
      return [];
    }
  });
  const [dismissedCorrectionIds, setDismissedCorrectionIds] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("dismissedCorrectionIds") || "[]");
    } catch {
      return [];
    }
  });
  ...
  useEffect(() => {
    const interval = setInterval(() => { fetchRequests(); }, 30000); // 30 sec
  ...
  const fetchRequests = async () => {
    // ✅ Fresh dismissed IDs localStorage se lo
    const freshDismissed = (() => {
      try {
        return JSON.parse(
          localStorage.getItem("dismissedCorrectionIds") || "[]",
        );
      } catch {
        return [];
      }
    })();
    setDismissedCorrectionIds(freshDismissed);
    setLoading(true);
    try {
      ...
      if (activeTab === "leave") {
        ...
        const response = await getMyCorrectionRequests(statusParam);
        if (response.success) {
          setCorrectionRequests(response.data?.correctionRequests || []);
        }
        // ✅ Total count ke liye hamesha all fetch karo
        if (filter !== "all") {
          try {
            const allRes = await getMyCorrectionRequests(null);
            if (allRes.success) {
              setAllCorrectionRequests(allRes.data?.correctionRequests || []);
            }
          } catch (e) {}
        } else {
          setAllCorrectionRequests(response.data?.correctionRequests || []);
        }
      }
    } catch (error) {
  ...
  const handleCancelLeave = async (requestId) => {
    ...
    try {
      const token = getToken();
      const response = await fetch(
        `${API_URL_LOCAL}/leave-requests/${requestId}/cancel`,
        {
          method: "PATCH",
          headers: {
    ...
    <div className="my-requests-container">
      ...
      <div className="tabs-container">
        <div className="tabs">
          ...
          >
            ...
            {allCorrectionRequests.filter(
              (r) => !dismissedCorrectionIds.includes(r._id),
            ).length || correctionRequests.length}
            )
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          {activeTab === "correction" && (
            <div className="requests-grid">
              {correctionRequests.filter((r) =>
                filter === "all"
                  ? !dismissedCorrectionIds.includes(r._id)
                  : true,
              ).length === 0 ? (
                <div className="empty-state">
                  <span className="empty-icon">✅</span>
                  ...
                  <p>
                    Submit a request if your attendance was marked incorrectly.
                  </p>
                  <button
                    className="btn btn-secondary"
                    onClick={() => setShowCorrectionModal(true)}
                ...
                correctionRequests
                  .filter((r) => {
                    if (filter === "all")
                      return !dismissedCorrectionIds.includes(r._id);
                  ...
                  .map((request) => (
                    <div
                      key={request._id}
                      className="request-card correction-card"
                      style={{ position: "relative" }}
                    >
                      {(request.status === "approved" ||
                        request.status === "rejected") && (
          ...
          const updated = [...dismissedCorrectionIds, id];
          setDismissedCorrectionIds(updated);
          localStorage.setItem('dismissedCorrectionIds', JSON.stringify(updated));
          setCorrectionRequests(prev => prev.filter(r => r._id !== id));
          setAllCorrectionRequests(prev => prev.filter(r => r._id !== id));
        }}
                          title="Dismiss"
                          style={{
                            position: "absolute",
                      ...
                      <div className="request-details">
                        ...
                        <div className="detail-row">
                          ...
                          <span className="detail-value">
                            ...
                            <span style={{ margin: "0 6px", color: "#94a3b8" }}>
                              →
                            </span>
                            <span
                              style={{
