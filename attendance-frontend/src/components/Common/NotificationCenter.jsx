...
const NotificationCenter = ({ userType = 'employee' }) => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (!token) return;
      const res = await fetch(`${API_URL}/notifications/my-notifications?limit=10`, {
  ...
  return (
    <>
      ...
      <div style={{ position: 'relative' }} ref={dropdownRef}>
        {/* Bell Button */}
        <button onClick={() => setIsOpen(!isOpen)} style={S.bellBtn} title="Notifications">
          🔔
          {unreadCount > 0 && (
          ...
          <div style={S.dropdown}>
            ...
            <div style={S.dropHeader}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <button
                  style={{ ...S.filterBtn, background: showUnreadOnly ? '#667eea' : 'white', color: showUnreadOnly ? 'white' : '#6b7280' }}
            ...
            <div style={S.listBox}>
              {loading ? (
                <div style={S.emptyBox}><div style={S.spinner}></div></div>
              ) : displayed.length === 0 ? (
...
const S = {
  ...
  notifMsg: {
    fontSize: 12, color: '#6b7280', marginBottom: 4, lineHeight: 1.4,
