...
const ManagerMarkAttendanceModal = ({ selectedDate, onClose, onAttendanceMarked }) => {
  ...
  const filterEmployees = () => {
    ...
    if (statusFilter === 'present') {
      ...
      filtered = filtered.filter(emp => emp.todayStatus === 'leave' || emp.todayStatus === 'on-leave');
    } else if (statusFilter === 'absent') {
      filtered = filtered.filter(emp =>
        !emp.todayStatus || (emp.todayStatus !== 'present' && emp.todayStatus !== 'leave' && emp.todayStatus !== 'on-leave')
      );
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!attendanceData.status) { alert('⚠️ Please select attendance status'); return; }
          ...
          <div style={{ display: 'grid', gridTemplateColumns: selectedEmployee ? '1fr 1fr' : '1fr', gap: 24, height: '100%' }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: statusFilter === f.key
                        ? (f.key === 'present' ? '#10b981' : f.key === 'leave' ? '#f59e0b' : '#667eea')
                        : 'white',
              ...
              <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8 }}>
                {loading ? (
                  ...
                  <div
                    ...
                    style={{
                      ...S.empRow,
                      background: selectedEmployee?._id === emp._id ? '#667eea22' : 'white',
                      border: `2px solid ${selectedEmployee?._id === emp._id ? '#667eea' : '#e5e7eb'}`,
                    }}
                    onMouseEnter={e => { if (selectedEmployee?._id !== emp._id) e.currentTarget.style.background = '#f3f4f6'; }}
                    onMouseLeave={e => { if (selectedEmployee?._id !== emp._id) e.currentTarget.style.background = 'white'; }}
                  >
                    ...
                    {emp.todayAttendance && (
                      <span style={S.checkedInBadge}>Checked In</span>
                    )}
                  </div>
                ))}
              </div>
            ...
            {selectedEmployee && (
              <div style={{ ...S.panel, overflowY: 'auto' }}>
                ...
                {/* Employee Info */}
                <div style={S.empInfoBox}>
                  <div style={S.empInfoAvatar}>
                ...
                {/* Status Select — only if NOT checked in */}
                {!selectedEmployee.todayAttendance && (
                ...
                {(attendanceData.status === 'present' || attendanceData.status === 'half-day' || selectedEmployee.todayAttendance) && (
                  <div style={S.timeBox}>
                    <div style={{ display: 'grid', gridTemplateColumns: selectedEmployee.todayAttendance ? '1fr' : '1fr 1fr', gap: 16, marginBottom: 12 }}>
                      ...
                      {/* Clock In — only if NOT already checked in */}
                      {!selectedEmployee.todayAttendance && (
                        <div>
...
const S = {
  ...
  filterBtn:   { padding:'8px 12px', border:'2px solid #e5e7eb', borderRadius:8, fontSize:13, fontWeight:600, cursor:'pointer' },

  empRow:   { borderRadius:12, padding:12, cursor:'pointer', transition:'all 0.2s', display:'flex', alignItems:'center', gap:12 },
  empAvatar:{ width:40, height:40, borderRadius:'50%', background:'linear-gradient(135deg,#667eea,#764ba2)', display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontSize:14, fontWeight:700, flexShrink:0 },
  ...
  empInfoAvatar:{ width:48, height:48, borderRadius:'50%', background:'linear-gradient(135deg,#667eea,#764ba2)', display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontSize:16, fontWeight:700, flexShrink:0 },
  ...
  select:   { width:'100%', padding:12, border:'2px solid #e5e7eb', borderRadius:10, fontSize:14, outline:'none', cursor:'pointer', background:'white', marginBottom:0 },
