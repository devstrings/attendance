...
const ClockInOut = () => {
    ...
    try {
      ...
      const combinedData = employees.map(emp => {
        ...
        return {
          ...
          clockIn: attendance?.clockIn ? new Date(attendance.clockIn).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }) : null,
          clockOut: attendance?.clockOut ? new Date(attendance.clockOut).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }) : null,
  ...
  const getShiftHours = () => {
    const startStr = systemConfig?.workingHours?.startTime || systemConfig?.shiftStartTime || '09:00';
    const endStr = systemConfig?.workingHours?.endTime || systemConfig?.shiftEndTime || '17:00';
  ...
  const formatWorkingDays = () => {
    ...
    const sorted = [...days].sort((a,b) => allDaysOrder.indexOf(a) - allDaysOrder.indexOf(b));
    // Check if consecutive
    let isConsecutive = true;
    for (let i = 1; i < sorted.length; i++) {
      if (allDaysOrder.indexOf(sorted[i]) - allDaysOrder.indexOf(sorted[i-1]) !== 1) {
  ...
  const formatOffDays = () => {
    ...
    const workDays = systemConfig?.workingDays || ['Monday','Tuesday','Wednesday','Thursday','Friday'];
  ...
  const handleClockIn = async (empId, employeeName) => {
    ...
    if (now.getHours() < startH) {
      alert(`⏰ Office opens at ${formatShiftTime(systemConfig?.workingHours?.startTime || '09:00')}. Please clock in after that.`);
      return;
    }
    if (now.getHours() >= endH) {
      alert(`⏰ Office hours ended at ${formatShiftTime(systemConfig?.workingHours?.endTime || '17:00')}. Cannot clock in now.`);
      return;
    }
    ...
    if (window.confirm(`Clock in for ${employeeName}?`)) {
      ...
      try {
        ...
        const response = await attendanceService.createAttendance({ employeeId: empId, date: today, clockIn: clockInTime, status: 'present' });
        if (response.success) {
  ...
  const handleClockOut = async (attendanceId, employeeName) => {
    ...
    if (window.confirm(`Clock out for ${employeeName}?`)) {
      ...
      try {
        ...
        const response = await attendanceService.updateAttendance(attendanceId, { clockOut: clockOutTime });
        if (response.success) {
          ...
          <div className="manager-content">
            <div style={{ display:'flex', justifyContent:'center', alignItems:'center', height:'400px' }}>
              <div style={{ textAlign:'center' }}>
                <div style={{ width:'50px', height:'50px', border:'4px solid #f3f3f3', borderTop:'4px solid #667eea', borderRadius:'50%', animation:'spin 1s linear infinite', margin:'0 auto 20px' }} />
      ...
      <div className="manager-layout">
        ...
        <div className="manager-content">
          <div className="page-header">
            <h1>Clock In/Out</h1>
            <small style={{ color:'#666', fontSize:'12px' }}>🔄 Auto-refreshing every 5 seconds</small>
          </div>

          {/* ✅ Off Day Banner - uses real config */}
          {isWeekend && (
            <div style={{ background:'#fff3cd', border:'1px solid #ffc107', borderRadius:'8px', padding:'15px', marginBottom:'20px', textAlign:'center' }}>
              <strong>🏖️ {todayName} - Office Closed</strong>
              <p style={{ margin:'5px 0 0', fontSize:'14px' }}>{formatOffDays()}</p>
            </div>
          )}
          ...
          <div className="stats-grid-small">
            <div className="stat-card-small blue"><span className="stat-label">Total Employees</span><span className="stat-value">{stats.total}</span></div>
            <div className="stat-card-small green"><span className="stat-label">Clocked In</span><span className="stat-value">{stats.clockedIn}</span></div>
            <div className="stat-card-small purple"><span className="stat-label">Clocked Out</span><span className="stat-value">{stats.clockedOut}</span></div>
            <div className="stat-card-small orange"><span className="stat-label">Not Yet In</span><span className="stat-value">{stats.notYetIn}</span></div>
          </div>
            ...
            <table className="data-table">
              ...
              <tbody>
                {todayAttendance.length > 0 ? (
                  todayAttendance.map((record) => (
                    <tr key={record.id}>
                      ...
                      <td className={record.clockIn ? 'text-green' : 'text-muted'}>
                        {record.isWeekend ? 'Off Day' : (record.clockIn || 'Not clocked in')}
                      </td>
                      <td className={record.clockOut ? 'text-blue' : 'text-muted'}>
                        {record.isWeekend ? '—' : (record.clockOut || 'Not clocked out')}
                      </td>
                      <td>{record.isWeekend ? '—' : calculateWorkingHours(record.clockIn, record.clockOut)}</td>
                      <td>
                      ...
                      <td>
                        {record.isWeekend ? (
                          ...
                          <button className="btn-small danger" onClick={() => handleClockOut(record.attendanceId, record.employeeName)} disabled={clockingIn}>Clock Out</button>
                        ) : record.status === 'not-clocked-in' ? (
                          <button className="btn-small success" onClick={() => handleClockIn(record.id, record.employeeName)} disabled={clockingIn}>Clock In</button>
                        ) : (
                  ...
                  <tr><td colSpan="7" className="no-data">No employees found under your supervision</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <div className="info-box">
            ...
            <ul>
              <li>🕙 <strong>Office Hours:</strong> {systemConfig ? `${formatShiftTime(systemConfig.workingHours?.startTime || systemConfig.shiftStartTime)} - ${formatShiftTime(systemConfig.workingHours?.endTime || systemConfig.shiftEndTime)}` : 'Loading...'}</li>
              <li>⏱️ <strong>Late Entry After:</strong> {systemConfig ? formatShiftTime(systemConfig.workingHours?.lateEntryTime || systemConfig.lateEntryTime) : 'Loading...'}</li>
              <li>📅 <strong>Working Days:</strong> {systemConfig ? formatWorkingDays() : 'Loading...'}</li>
              <li>🏖️ <strong>Off Days:</strong> {systemConfig ? formatOffDays() : 'Loading...'}</li>
