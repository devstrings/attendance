...
const getWelcomeEmailTemplate = ({ firstName, lastName, email, password, employeeCode, department, designation, joiningDate }) => {
...
// ==================== SAME FOR MANAGER ====================
const getManagerWelcomeEmailTemplate = ({ firstName, lastName, email, password, department, designation }) => {
  return `
<!DOCTYPE html>
<html>
<body style="margin:0;padding:0;background:#f0f4f8;font-family:'Segoe UI',Arial,sans-serif;">
...
const getDashboard = async (req, res) => {
  try {
    ...
    let todayAttendance = 0, absentToday = 0, leaveToday = 0;
    if (isWorkingDay) {
      todayAttendance = await Attendance.countDocuments({ employeeId: { $in: activeEmployeeIds }, date: { $gte: today, $lt: tomorrow }, status: { $in: ['present', 'half-day', 'late'] } });
      leaveToday = await Attendance.countDocuments({ employeeId: { $in: activeEmployeeIds }, date: { $gte: today, $lt: tomorrow }, status: { $in: ['leave', 'on-leave'] } });
      absentToday = Math.max(0, totalEmployees - todayAttendance - leaveToday);
    }
    const pendingLeaves = await LeaveRequest.countDocuments({ status: 'pending', employee: { $in: activeEmployeeIds } });
    const recentEmployees = await Employee.find({ isActive: true }).sort({ createdAt: -1 }).limit(5).populate('userId', 'email isActive').populate('managerId', 'firstName lastName');
    res.status(200).json({ success: true, data: { stats: { totalEmployees, totalManagers, todayAttendance, absentToday, leaveToday, pendingLeaves }, recentEmployees, meta: { isWorkingDay, todayDayName, workingDays: systemConfig?.workingDays || [] } } });
  } catch (error) {
...
const createManager = async (req, res) => {
  try {
    const { email, password, firstName, lastName, phoneNumber, cnic, dateOfBirth, address, joiningDate, department, designation, salary, bankDetails, emergencyContact } = req.body;
    if (!email || !password || !firstName || !lastName || !phoneNumber) return res.status(400).json({ success: false, message: 'Required fields missing.' });
    ...
    if (cnic) { const existingCNIC = await Manager.findOne({ cnic, isActive: true }); if (existingCNIC) return res.status(400).json({ success: false, message: 'Manager with this CNIC already exists.' }); }
    const user = new User({ email: email.toLowerCase(), password, role: 'manager', isActive: true, createdBy: req.user.userId });
    await user.save();
    const manager = new Manager({ userId: user._id, firstName, lastName, phoneNumber, cnic: cnic || '', dateOfBirth, address: address || '', joiningDate: joiningDate || new Date(), department: department || 'General', designation: designation || 'Manager', salary: salary || 0, bankDetails, emergencyContact });
    await manager.save();
    ...
    try {
      await sendEmail({
        ...
        html: getManagerWelcomeEmailTemplate({ firstName, lastName, email: user.email, password, department: department || 'General', designation: designation || 'Manager' })
      });
      console.log(`✅ Manager welcome email sent to ${user.email}`);
    } catch (emailError) {
      console.error('⚠️ Manager welcome email failed (non-fatal):', emailError.message);
    }
...
const createEmployee = async (req, res) => {
  try {
    const { email, password, firstName, lastName, phoneNumber, cnic, dateOfBirth, address, managerId, department, designation, employmentType, salary, joiningDate, workSchedule } = req.body;
    if (!email || !password || !firstName || !lastName || !phoneNumber) return res.status(400).json({ success: false, message: 'Email, password, first name, last name, and phone number are required.' });
    if (!validateEmail(email)) return res.status(400).json({ success: false, message: 'Invalid email format.' });
    const existingUser = await User.findOne({ email: email.toLowerCase() });
    if (existingUser) return res.status(400).json({ success: false, message: 'Email already registered.' });
    if (managerId) { const manager = await Manager.findById(managerId); if (!manager) return res.status(404).json({ success: false, message: 'Manager not found.' }); }
...
if (!isUnique) {
  ...
  employeeCode = `EMP${Date.now().toString().slice(-6)}`;
    ...
    if (managerId) await Manager.findByIdAndUpdate(managerId, { $addToSet: { employeesUnder: employee._id } });
    ...
    if (managerId) {
      try {
        ...
        const assignedManager = await Manager.findById(managerId).populate('userId', 'email');
      ...
      <table width="580" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="background:#f9fafb;padding:16px;text-align:center;border-top:1px solid #e5e7eb;">
            ...
            console.log(`✅ Manager assignment email sent to ${assignedManager.userId.email}`);
          } catch (managerEmailErr) {
            console.error('⚠️ Manager email failed (non-fatal):', managerEmailErr.message);
          }
        }
      } catch (notifError) {
        console.error('⚠️ Manager notification failed (non-fatal):', notifError.message);
      }
    }
...
const getAllManagers = async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to fetch managers.', error: error.message }); }
};
...
const getAllEmployees = async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to fetch employees.', error: error.message }); }
};
...
const getUserDetails = async (req, res) => {
  try {
    ...
    if (profile) { const userIdValue = profile.userId?._id || profile.userId; const user = await User.findById(userIdValue).select('-password'); if (user) return res.status(200).json({ success: true, data: { user, profile } }); }
    const user = await User.findById(userId).select('-password');
    if (user) { profile = await ProfileModel.findOne({ userId: user._id }).populate(populateField, populateSelect); if (profile) return res.status(200).json({ success: true, data: { user, profile } }); }
    res.status(404).json({ success: false, message: 'User not found.' });
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to fetch user details.', error: error.message }); }
};
...
const updateUser = async (req, res) => {
  try {
    ...
    if (updateData.email && updateData.email !== user.email) { const existing = await User.findOne({ email: updateData.email.toLowerCase(), _id: { $ne: user._id } }); if (existing) return res.status(400).json({ success: false, message: 'Email already in use.' }); user.email = updateData.email.toLowerCase(); await user.save(); }
  ...
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to update user.', error: error.message }); }
};
...
const deleteUser = async (req, res) => {
  try {
    ...
    let profile = await ProfileModel.findById(userId);
    let user = null;
    if (!profile) { user = await User.findById(userId); if (!user) return res.status(404).json({ success: false, message: 'User not found.' }); profile = await ProfileModel.findOne({ userId: user._id }); } else { user = profile.userId ? await User.findById(profile.userId) : null; }
    if (userType === 'manager' && profile?.employeesUnder?.length > 0) return res.status(400).json({ success: false, message: `Cannot delete manager. ${profile.employeesUnder.length} employee(s) assigned.` });
    if (userType === 'employee' && profile) { await Attendance.deleteMany({ employeeId: profile._id }); await Leave.deleteMany({ employeeId: profile._id }); await Salary.deleteMany({ employeeId: profile._id }); if (profile.managerId) await Manager.findByIdAndUpdate(profile.managerId, { $pull: { employeesUnder: profile._id } }); }
  ...
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to delete user.', error: error.message }); }
};
...
const getAllAttendance = async (req, res) => {
  try {
    ...
    const query = {};
    if (date) { const d = new Date(date); d.setHours(0,0,0,0); query.date = { $gte: d, $lt: new Date(d.getTime() + 86400000) }; }
    if (employeeId) query.employeeId = employeeId;
    if (status) query.status = status;
    const records = await Attendance.find(query).populate({ path: 'employeeId', select: 'firstName lastName employeeCode department designation phoneNumber', populate: { path: 'userId', select: 'email' } }).populate('managerId', 'firstName lastName').sort({ date: -1 }).limit(limit * 1).skip((page - 1) * limit);
  ...
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to fetch attendance.', error: error.message }); }
};
...
const manageHoliday = async (req, res) => {
  try {
    ...
    if (holidayId) { const holiday = await Holiday.findByIdAndUpdate(holidayId, { $set: obj }, { new: true }); if (!holiday) return res.status(404).json({ success: false, message: 'Holiday not found.' }); return res.status(200).json({ success: true, message: 'Holiday updated.', data: { holiday } }); }
    const start = new Date(holidayDate); start.setHours(0,0,0,0); const end = new Date(holidayDate); end.setHours(23,59,59,999);
    ...
    const holiday = new Holiday(obj); await holiday.save();
    res.status(201).json({ success: true, message: 'Holiday created.', data: { holiday } });
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to manage holiday.', error: error.message }); }
};
...
const getSummaryReport = async (req, res) => {
  try {
    ...
    const query = {};
    if (startDate || endDate) { query.date = {}; if (startDate) { const s = new Date(startDate); s.setHours(0,0,0,0); query.date.$gte = s; } if (endDate) { const e = new Date(endDate); e.setHours(23,59,59,999); query.date.$lte = e; } }
    if (employeeId) query.employeeId = employeeId;
    const records = await Attendance.find(query).populate({ path: 'employeeId', select: 'firstName lastName employeeCode department designation', populate: { path: 'userId', select: 'email' } }).populate('managerId', 'firstName lastName').sort({ date: -1 });
    let filtered = records;
    if (department) filtered = records.filter(r => r.employeeId?.department === department);
    res.status(200).json({ success: true, data: { records: filtered, statistics: { total: filtered.length, present: filtered.filter(r => r.status === 'present').length, absent: filtered.filter(r => r.status === 'absent').length, late: filtered.filter(r => r.isLate === true).length, leave: filtered.filter(r => ['leave', 'on-leave'].includes(r.status)).length } } });
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to fetch report.', error: error.message }); }
};

const getAllHolidays = async (req, res) => {
  try {
    const { year } = req.query;
    const holidays = await Holiday.find(year ? { year: parseInt(year) } : {}).sort({ date: 1 }).populate('createdBy', 'email');
    res.status(200).json({ success: true, data: { holidays } });
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to fetch holidays.', error: error.message }); }
};
...
const deleteHoliday = async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to delete holiday.', error: error.message }); }
};
...
const getMonthlyConfig = async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to fetch config.', error: error.message }); }
};

const updateMonthlyConfig = async (req, res) => {
  try {
    const { configId } = req.params;
    if (configId) { const config = await MonthlyConfig.findByIdAndUpdate(configId, { $set: req.body }, { new: true }); if (!config) return res.status(404).json({ success: false, message: 'Config not found.' }); return res.status(200).json({ success: true, message: 'Config updated.', data: { config } }); }
    const config = new MonthlyConfig({ ...req.body, createdBy: req.user.userId }); await config.save();
...
const getEmployeeById = async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to fetch employee.', error: error.message }); }
};
...
const getAllLeaves = async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to fetch leaves.', error: error.message }); }
};
...
const getSettings = async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to fetch settings.', error: error.message }); }
};
...
const forceDeleteEmployee = async (req, res) => {
  try {
    ...
    if (employee) { if (employee.userId) await User.findByIdAndDelete(employee.userId); if (employee.managerId) await Manager.findByIdAndUpdate(employee.managerId, { $pull: { employeesUnder: employee._id } }); return res.status(200).json({ success: true, message: 'Employee deleted.' }); }
    res.status(404).json({ success: false, message: 'Employee not found' });
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to delete.', error: error.message }); }
};

const getSystemConfig = async (req, res) => {
  try {
    let config = await SystemConfig.findOne({ isActive: true }).populate('createdBy', 'email').populate('updatedBy', 'email');
    if (!config) { config = new SystemConfig({ workingDays: ['Monday','Tuesday','Wednesday','Thursday','Friday'], workingHours: { startTime: '10:00', endTime: '19:00', lateEntryTime: '10:30' }, breakTime: 60, leavePolicy: { allowedLeaves: 2, autoAbsentOnExceed: true }, weekendDays: ['Saturday','Sunday'], isActive: true, createdBy: req.user.userId }); await config.save(); }
...
const createSystemConfig = async (req, res) => {
  try {
    ...
    const config = new SystemConfig({ ...req.body, isActive: true, createdBy: req.user.userId, effectiveFrom: new Date() }); await config.save();
    res.status(201).json({ success: true, message: 'Config created.', data: { config } });
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to create config.', error: error.message }); }
};
...
const fixEmployeeManagerLinks = async (req, res) => {
  try {
    ...
    for (const emp of employees) {
      ...
      if (!linked) { await Manager.findByIdAndUpdate(emp.managerId, { $addToSet: { employeesUnder: emp._id } }); fixed++; } else alreadyLinked++;
    }
    res.status(200).json({ success: true, message: `Fixed: ${fixed}, Already linked: ${alreadyLinked}`, data: { fixed, alreadyLinked } });
  } catch (error) { res.status(500).json({ success: false, message: 'Failed to fix links.', error: error.message }); }
};

module.exports = {
  getDashboard, createManager, createEmployee,
  getAllManagers, getAllEmployees, getEmployeeById,
  getUserDetails, updateUser, deleteUser,
  getAllAttendance, manageHoliday, getAllHolidays, deleteHoliday,
  getMonthlyConfig, updateMonthlyConfig,
  getAllLeaves, getSettings, getSummaryReport,
  forceDeleteEmployee, getSystemConfig, createSystemConfig,
  updateSystemConfig, fixEmployeeManagerLinks
};