...
exports.getLeavePolicy = async (req, res) => {
  try {
    ...
    if (!employee) return res.status(404).json({ success: false, message: 'Employee profile not found. Please contact admin.' });
...
exports.getMyLeaveRequests = async (req, res) => {
  try {
    ...
    try { stats = await LeaveRequest.getEmployeeLeaveStats(employee._id, year || currentYear); } catch (e) {}
...
exports.approveLeaveRequest = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'Leave approved, attendance marked, notifications sent', data: leaveRequest });
  } catch (error) {
